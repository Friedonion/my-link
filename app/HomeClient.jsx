...
const LandingView = ({ onLogin }) => {
  return (
    <div className="flex flex-col items-center w-full max-w-5xl mx-auto">
      {/* Hero Section - Restored to Original Simple Style */}
      <section className="flex flex-col items-center justify-center text-center py-20 px-4 gap-8 w-full min-h-[60vh]">
        ...
        <div>
          <h1 className="text-4xl sm:text-5xl md:text-6xl font-extrabold text-zinc-900 dark:text-zinc-50 tracking-tight">
            마이링크
          </h1>
          <p className="mt-4 text-lg md:text-xl text-zinc-600 dark:text-zinc-400 font-medium max-w-2xl">
            하나의 링크로 당신의 모든 것을 공유하세요.<br className="hidden sm:block" />
            인스타그램, 유튜브, 블로그까지 한 페이지에 담을 수 있습니다.
          </p>
        </div>
      ...
      {/* Features Section - Simplified and Unified Design */}
      <section className="w-full py-20 px-4 border-t border-zinc-100 dark:border-zinc-900">
        <div className="text-center mb-16">
          <h2 className="text-2xl md:text-3xl font-bold text-zinc-900 dark:text-zinc-50 tracking-tight">서비스 특징</h2>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          <FeatureCard 
            icon={<Zap className="h-5 w-5" />}
            title="초고속 설정"
            description="로그인 즉시 나만의 페이지가 생성됩니다."
          />
          <FeatureCard 
            icon={<TrendingUp className="h-5 w-5" />}
            title="실시간 통계"
            description="링크 클릭 수를 실시간으로 확인하세요."
          />
          <FeatureCard 
            icon={<Layout className="h-5 w-5" />}
            title="직관적인 편집"
            description="화면에서 직접 클릭하여 수정할 수 있습니다."
          />
          <FeatureCard 
            icon={<Smartphone className="h-5 w-5" />}
            title="모바일 최적화"
            description="모든 기기에서 완벽한 레이아웃을 제공합니다."
          />
          <FeatureCard 
            icon={<Globe className="h-5 w-5" />}
            title="자동 파비콘"
            description="URL만 입력하면 아이콘을 자동으로 찾아줍니다."
          />
          <FeatureCard 
            icon={<Shield className="h-5 w-5" />}
            title="안전한 관리"
            description="강력한 보안으로 데이터를 안전하게 보호합니다."
          />
        </div>
      </section>
      ...
      <section className="w-full py-24 px-4 text-center">
        <div className="max-w-xl mx-auto space-y-8">
          ...
          <Button 
            size="lg" 
            className="rounded-full px-12 py-6 font-bold bg-zinc-900 text-white dark:bg-zinc-50 dark:text-zinc-900 hover:opacity-90 transition-all"
            onClick={onLogin}
          >
            무료로 만들기
          </Button>
...
const FeatureCard = ({ icon, title, description }) => (
  <div className="flex flex-col items-center text-center p-6 gap-4">
    <div className="h-12 w-12 rounded-2xl bg-zinc-100 dark:bg-zinc-900 flex items-center justify-center text-zinc-900 dark:text-zinc-100">
      {icon}
    </div>
